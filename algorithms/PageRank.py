...
REQUIRED arguments:
    NETWORK - A tab-delimited file with one interaction per line. Each
        line should have at least 2 columns: tail, head. Edges are directed from
        tail->head if --undirected is not specified; otherwise each undirected
        edge is converted to a bi-directed edge. This file can optionally have 
        a third column specifying the edge weight.
'''
    parser = OptionParser(usage=usage)

    # General Options
    parser.add_option('-u', '--undirected', action='store_true', default=False,\
        help='NETWORK file is undirected (default is False).')
    ...
    # Teleporation Parameter Group
    group = OptionGroup(parser,'Teleportation Options')
    group.add_option('-t','--tele-node', action='append',type='string',default=[],\
        help='Source node for teleporting. Can specify multiple; uniform weights for these '+
        'nodes will be used.  This is an alternative to passing a file of tele-weights (below),'+
        'or not specifying any nodes (uniform probs are used).')

    group.add_option('', '--tele-weights', type='string', default=None, metavar='STR',\
        help='Incoming teleportation weights for each node, in a tab-separated file ' + \
        'with the form "nodename[tab]weight". This is an alternative to passing in separate nodes (above),' +
        'or not specifying any nodes (uniform probs are used).')
    ...
    if len(opts.tele_node) > 0 and opts.tele_weights:
        parser.print_help()
        sys.exit('\nERROR: Only one of --tele-node and --tele-weights must be specified. Exiting.')
    ...
    for line in infile:
        ...
        if opts.undirected:
            net.add_edge(id2, id1, weight=eWeight)
    ...
    if opts.tele_weights != None:
        ...
        for line in infile:
            ...
            teleProbs[node] = weight
    elif len(opts.tele_node) > 0:

        print('\nSetting uniform edge weights for the %d input nodes' % (len(opts.tele_node)))
        for node in opts.tele_node:
            teleProbs[node] = 1.0/len(opts.tele_node)
